...
export function Sidebar({ collapsed }: SidebarProps) {
  ...
  return (
    <aside
      className={`${w} shrink-0 border-r border-sidebar-border bg-sidebar text-sidebar-foreground flex flex-col transition-all duration-200 ease-in-out overflow-hidden h-full`}
    >
      {/* Sidebar brand strip */}
      <div className={`flex h-14 items-center border-b border-sidebar-border shrink-0 ${collapsed ? "justify-center px-2" : "px-4 gap-2"}`}>
        <Wheat className="w-4 h-4 text-accent shrink-0" />
        {!collapsed && <span className="tokenharvest-wordmark text-lg text-white truncate">Grain Market</span>}
      </div>
