...
import { useGetForwardContract, useCoSignForwardContract, useResolveDefault, useGetMe } from "@workspace/api-client-react";
...
import { ArrowLeft, FileSignature, AlertTriangle } from "lucide-react";
...
export default function ForwardDetail() {
  ...
  const isEnablerOrSeller = ["ENABLER"].includes(me?.tier ?? "") || contract?.sellerId === me?.id;
  ...
  const handleResolveDefault = async () => {
    try {
      await resolveDefault({ contractId });
      toast({ title: "Default resolved", description: "Buyer bond forfeited to seller." });
      refetch();
    } catch (err: any) {
  ...
  const canResolveDefault = (isEnablerOrSeller || contract.sellerId === me?.id) &&
    contract.contractStatus === "ACTIVE" && isMatured;
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div className="p-3 rounded-lg bg-muted/40 text-center">
                ...
                {isMatured && <p className="text-xs text-red-600 font-medium mt-1">Matured</p>}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className={`flex justify-between items-center p-2 rounded border ${BOND_COLORS[contract.sellerBondStatus]}`}>
                  ...
                  <span className="font-bold text-xs">{contract.sellerBondStatus.replace("_", " ")}</span>
                </div>
                <div className={`flex justify-between items-center p-2 rounded border ${BOND_COLORS[contract.buyerBondStatus]}`}>
                  ...
                  <span className="font-bold text-xs">{contract.buyerBondStatus.replace("_", " ")}</span>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      By co-signing, you commit a performance bond of <strong>${parseFloat(String(contract.performanceBondUsd)).toFixed(2)}</strong> (15% of delivery price).
            ...
            {canResolveDefault && (
              <div className="p-4 rounded-lg bg-red-50 border border-red-200 space-y-3">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-sm text-red-800">Contract is matured and unresolved</p>
                    <p className="text-xs text-red-700 mt-1">
                      Triggering default will forfeit the buyer's bond of <strong>${parseFloat(String(contract.performanceBondUsd)).toFixed(2)}</strong> to the seller and release the eWR.
                    </p>
                  </div>
                </div>
                <Button variant="destructive" className="w-full" onClick={handleResolveDefault} disabled={isResolving}>
                  {isResolving ? "Resolving…" : "Resolve Default"}
                </Button>
              </div>
            )}
          </CardContent>
