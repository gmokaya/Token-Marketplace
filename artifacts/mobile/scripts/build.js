...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  // Derive the Clerk publishable key from the managed secret if not already set.
  // EXPO_PUBLIC_* vars must be present at Metro bundle time so they are inlined
  // into the JS bundle — setting them only at serve-time is too late.
  ...
  if (!clerkPublishableKey) {
    console.warn(
      'WARNING: CLERK_PUBLISHABLE_KEY / EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY is not set. ' +
        'Authentication will not work in the built app.',
    );
  }
