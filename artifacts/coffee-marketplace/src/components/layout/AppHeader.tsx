...
function useDarkMode() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("th-coffee-theme");
    if (saved === "dark") {
  ...
  const toggle = () => {
    ...
    localStorage.setItem("th-coffee-theme", nowDark ? "dark" : "light");
    setDark(nowDark);
  };
    ...
    <header className="sticky top-0 z-50 flex items-center justify-between h-14 pl-4 pr-3 bg-card border-b border-border shrink-0">
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          className="flex items-center justify-center w-8 h-8 text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
        >
          {collapsed
        ...
        <Link href="/">
          <button
            ...
            className="flex items-center gap-1.5 h-8 px-2.5 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
          >
      ...
      <div className="flex items-center gap-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => signOut()}
            aria-label="Sign out"
            className="flex items-center justify-center w-8 h-8 text-muted-foreground hover:text-destructive hover:bg-muted transition-colors"
          >
