...
function KybReminder() {
  ...
  if (dismissed || !user || user.tier === "ADMIN" || user.kybStatus === "VERIFIED") return null;
...
export function Layout({ children }: LayoutProps) {
  const [collapsed, setCollapsed] = useState(() => {
    try { return localStorage.getItem("th-coffee-sidebar-collapsed") === "true"; }
    catch { return false; }
  });
  ...
  const handleToggle = () => {
    setCollapsed((prev) => {
      const next = !prev;
      try { localStorage.setItem("th-coffee-sidebar-collapsed", String(next)); } catch {}
      return next;
    });
  };
