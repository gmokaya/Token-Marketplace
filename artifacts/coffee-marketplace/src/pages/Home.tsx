...
export default function Home() {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden flex flex-col select-none">
      ...
      {/* Layered gradient: shared marketplace treatment */}
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-end px-8 md:px-14 pb-8">
        ...
        <div className="inline-flex items-center gap-2 bg-white/10 text-white/75 text-xs font-medium px-3 py-1.5 rounded-full mb-4 w-fit backdrop-blur-sm border border-white/15">
          ...
          B2B Specialty Coffee Marketplace
        </div>

        <h1 className="text-5xl md:text-7xl font-bold text-white leading-[1.0] tracking-tight max-w-3xl">
          Where specialty coffee
          <br />
          <span className="text-white">meets global buyers.</span>
        </h1>

        <p className="mt-3 text-white/55 text-sm md:text-base leading-relaxed max-w-xl">
          Every exceptional coffee begins with its origin. TokenHarvest is the trusted marketplace for
          specialty and commercial green coffee, connecting producers, brokers, and roasters through
          verified provenance, electronic warehouse receipts, and embedded trade finance.
        </p>
        ...
        <div className="mt-5 flex items-center gap-4">
          <Link href={isSignedIn ? "/dashboard" : "/sign-in"}>
            ...
            >
              {isSignedIn ? "Go to Terminal" : "Sign in to Terminal"} <ArrowRight className="w-4 h-4" />
            </button>
          </Link>
          <Link href="/sign-up">
            <button
