...
export function Layout({ children }: { children: ReactNode }) {
  ...
  const [collapsed, setCollapsed] = useState(() => {
    try {
      return localStorage.getItem("th-tea-sidebar-collapsed") === "true";
    } catch {
      return false;
    }
  });
  ...
  const handleToggle = () => {
    setCollapsed((current) => {
      const next = !current;
      try {
        localStorage.setItem("th-tea-sidebar-collapsed", String(next));
      } catch {}
      return next;
    });
  };
  ...
  return (
    <div className="market-app-shell market-app-shell--tea">
      <Show when="signed-in">
