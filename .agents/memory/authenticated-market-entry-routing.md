...
Resolve the signed-in user's profile and onboarding state before choosing the authenticated landing route. A completed non-admin account goes directly to its market; a new account goes to onboarding; admins retain the dashboard entry without requiring an onboarding record. Authenticated profile reads must return fresh JSON, not conditional-cache-only responses.

**Why:** Sending every signed-in account through onboarding creates a visible wizard flash, while admins intentionally have no onboarding record and receive a 404. A `304` response has no body for the client to hydrate, which can make a completed profile appear missing.
